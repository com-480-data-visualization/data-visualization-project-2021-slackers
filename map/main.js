  ...
  constructor() {
    ...
    Promise.all([
      ...
      this.addHistogram = function (e) {
        ...
        //const byTrait = crossfilter(data).dimension((d) => d.agre);
        //data = byCountry.filter(byTrait).top(Infinity);

        const agreScores = [];
        const ages = [];

        data.forEach((row) => {
          agreScores.push(row["agre"]);
          ages.push(row["age"]);
        });
        ...
        var x = d3
          .scaleLinear()
          .domain([0, 99]) // can use this instead of 1000 to have the max of data: d3.max(data, function(d) { return +d.price })
          .range([0, width]);
        svg
          .append("g")
